  ...
  class Events
    ...
    def event_query
      "
        ...
        ,(
          SELECT 
          CASE WHEN iphdr.ip_proto=2 THEN 'tcp'
              WHEN iphdr.ip_proto=3 THEN 'udp'
              ELSE 'icmp'
          END as \"protocol\"
        )
        ,iphdr.ip_src
        ,(
          SELECT 
          CASE WHEN iphdr.ip_proto=2 THEN (select tcp_sport from tcphdr where tcphdr.sid = event.sid and tcphdr.cid = event.cid)
              WHEN iphdr.ip_proto=3 THEN (select udp_sport from udphdr where udphdr.sid = event.sid and udphdr.cid = event.cid)
              ELSE 0
          END as \"sport\"
        )
        ,iphdr.ip_dst
        ,(
          SELECT 
          CASE WHEN iphdr.ip_proto=2 THEN (select tcp_dport from tcphdr where tcphdr.sid = event.sid and tcphdr.cid = event.cid)
              WHEN iphdr.ip_proto=3 THEN (select udp_dport from udphdr where udphdr.sid = event.sid and udphdr.cid = event.cid)
              ELSE 0
          END as \"dport\"
        )
        FROM event
